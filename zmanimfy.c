...
// zmanim= fopen(months[monprn[j]- 1],"w");
		   ...
		   for (j = 0; j <=11; j++)
		 {
		    ...
		    fprintf(zmanim,"\tM\"A Zman Tfillah, SUBTRACT 24 min.");
...

{
   // _clearscreen(_GCLEARSCREEN);
    ...
    printf("\r\nWelcome to Zmanim Ver. 1.6!\r\n(C) 1991-1999 Shore Tech,Howard/Chaim Joseph Ochs\r\n1121A Sage St.Far Rockaway, NY 11691 (877) 613-7826\r\n\r\n");
...
void getzm(int daynum)
{
 ...
 lonh = lon / 15;
  ...
  if(fr_da_ap == 1)
      {
	if(daynum > 74 && daynum < 308 + get_leap(year) )
	  srf = srf + 1;
      }
  else if( daynum >  ( 63
	  + get_leap(year) ) + (7 - fr_da_ap) % 7 && daynum < (311 + get_leap(year) ) - (la_da_oc - 1) )
	  srf = srf + 1;
  if(srf<= 0)
    srf+= 12;
  ...
  srf = srf * 60;
   ...
   else if( daynum >  ( 67 + get_leap(year) ) + (7 - fr_da_ap) % 7 && daynum < (311 + get_leap(year) ) - (la_da_oc - 1) )
	   ssf = ssf + 1;
   if(ssf< 12 )
      ssf+= 12;
   ssf = ssf * 60;
